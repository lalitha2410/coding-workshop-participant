"""
API tests — the auth function.handler router with the repository mocked (via the
shared `repo` fixture). Covers routing, status codes, request parsing, response
shaping, and business rules (self-demotion, self-delete, duplicate/validation
400s). Token/hash logic runs for real (auth is stateless), so these need no
database. The 401/403 permission variants live in test_security.py.
"""
...
def test_register_default_viewer_returns_201(repo):
    ...
    assert parse_body(resp)["role"] == "Viewer"
    # password must never be echoed back
    assert "password" not in parse_body(resp) and "password_hash" not in parse_body(resp)
...
def test_register_duplicate_returns_400(repo):
    ...
    resp = function.handler(event)
    assert resp["statusCode"] == 400
    assert "already in use" in parse_body(resp)["error"]
...
def test_register_unknown_role_returns_400(repo):
    ...
    resp = function.handler(event)
    assert resp["statusCode"] == 400
    assert "Unknown role" in parse_body(resp)["error"]
...
def test_login_wrong_password_returns_401(repo):
    ...
    assert parse_body(resp)["error"] == "Invalid credentials."
...
def test_me_returns_current_user(repo):
    ...
    resp = function.handler(event)
    assert resp["statusCode"] == 200
    assert parse_body(resp)["username"] == "ada"
...
# ---------------------------------------------------------------------------
# Admin user management — /auth/users (list)
# ---------------------------------------------------------------------------

def test_list_users_admin_200(repo, bypass_user_check):
    repo.set("list_users", {"items": [A_USER], "total": 1, "limit": 50, "offset": 0})
    resp = function.handler({"httpMethod": "GET", "path": "/auth/users", "headers": bearer("Admin")})
    assert resp["statusCode"] == 200
    body = parse_body(resp)
...
def test_list_users_search_passthrough(repo, bypass_user_check):
    ...
    function.handler({"httpMethod": "GET", "path": "/auth/users", "headers": bearer("Admin"),
...
def test_change_role_200(repo, bypass_user_check):
    ...
    resp = function.handler({"httpMethod": "PUT", "path": "/auth/users/5/role", "headers": bearer("Admin"),
...
def test_change_role_invalid_role_400(repo, bypass_user_check):
    ...
    assert resp["statusCode"] == 400
    assert "must be one of" in parse_body(resp)["error"]
...
def test_change_role_self_demotion_400(repo, bypass_user_check):
    ...
    resp = function.handler({"httpMethod": "PUT", "path": "/auth/users/5/role", "headers": bearer("Admin", sub="5"),
                             "body": json.dumps({"role": "Viewer"})})
    assert resp["statusCode"] == 400
    assert "own role" in parse_body(resp)["error"]
...
def test_change_role_not_found_404(repo, bypass_user_check):
    ...
    resp = function.handler({"httpMethod": "PUT", "path": "/auth/users/999/role", "headers": bearer("Admin"),
                             "body": json.dumps({"role": "Manager"})})
    assert resp["statusCode"] == 404
...
def test_delete_user_204(repo, bypass_user_check):
    ...
    resp = function.handler({"httpMethod": "DELETE", "path": "/auth/users/5", "headers": bearer("Admin")})
    assert resp["statusCode"] == 204 and resp["body"] == ""


def test_delete_self_400(repo, bypass_user_check):
    resp = function.handler({"httpMethod": "DELETE", "path": "/auth/users/5", "headers": bearer("Admin", sub="5")})
    assert resp["statusCode"] == 400
    assert "own account" in parse_body(resp)["error"]


def test_delete_user_not_found_404(repo, bypass_user_check):
    repo.set("delete_user", None)
    resp = function.handler({"httpMethod": "DELETE", "path": "/auth/users/999", "headers": bearer("Admin")})
    assert resp["statusCode"] == 404


def test_users_wrong_method_405(repo, bypass_user_check):
    resp = function.handler({"httpMethod": "POST", "path": "/auth/users", "headers": bearer("Admin")})
    assert resp["statusCode"] == 405


# ---------------------------------------------------------------------------
# Admin user management — update details (username/email)
# ---------------------------------------------------------------------------
...
def test_update_user_details_200(repo, bypass_user_check):
    ...
    resp = function.handler({"httpMethod": "PUT", "path": "/auth/users/5", "headers": bearer("Admin"),
...
def test_update_user_invalid_email_400(repo, bypass_user_check):
    ...
    assert resp["statusCode"] == 400
    assert any("email" in d for d in parse_body(resp)["details"])
...
def test_update_user_not_found_404(repo, bypass_user_check):
    ...
    resp = function.handler({"httpMethod": "PUT", "path": "/auth/users/999", "headers": bearer("Admin"),
